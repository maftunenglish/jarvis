...
class OpenAIClient:
    def __init__(
        ...
        max_tokens_per_request: int = 2000,  # Limit per single request
    ):
        load_dotenv()
        self.model = model
        ...
        self.token_usage_file = Path("openai_token_usage.json")
        self.token_usage_data = self._load_token_usage_data()
        print(
            f"[openai_client] Token usage: {self.token_usage_data['tokens_used_today']}/{self.daily_token_limit} today"
        )
    ...
    def _load_token_usage_data(self) -> Dict:
        ...
        try:
            ...
            print(f"[openai_client] Error loading token usage: {e}")

        return default_data

    def _save_token_usage_data(self):
        """Save token usage data to file."""
        try:
            with open(self.token_usage_file, "w") as f:
                json.dump(self.token_usage_data, f, indent=2)
        except Exception as e:
            print(f"[openai_client] Error saving token usage: {e}")

    def _check_token_limits(self) -> Tuple[bool, str]:
        """Check if token usage is within limits."""
        if self.token_usage_data["tokens_used_today"] >= self.daily_token_limit:
            return (
                False,
                f"Daily token limit reached ({self.token_usage_data['tokens_used_today']}/{self.daily_token_limit})",
            )
    ...
    def _truncate_messages_if_needed(
        ...
        print(
            f"[openai_client] Warning: Message too long ({estimated_tokens} tokens), truncating..."
        )
        ...
        for message in messages:
            if message["role"] == "system":
                ...
                sys_tokens = self._estimate_message_tokens([message])
                remaining_tokens -= sys_tokens
            else:
                # Truncate user/assistant messages
                content = message["content"]
                max_chars = remaining_tokens * 4  # Approx 4 chars per token
                ...
                if len(content) > max_chars:
                    ...
                    truncated_messages.append(message)
                    remaining_tokens -= self._estimate_message_tokens([message])
    ...
    def get_token_usage_stats(self) -> Dict:
        ...
        return {
            "tokens_used_today": self.token_usage_data["tokens_used_today"],
            "daily_token_limit": self.daily_token_limit,
            "tokens_remaining_today": max(
                0, self.daily_token_limit - self.token_usage_data["tokens_used_today"]
            ),
            "tokens_used_total": self.token_usage_data["tokens_used_total"],
            "request_count_today": self.token_usage_data["request_count_today"],
            "request_count_total": self.token_usage_data["request_count_total"],
            "last_reset_date": self.token_usage_data["last_reset_date"],
        }
    ...
    def chat_completion(
        ...
        can_proceed, reason = self._check_token_limits()
        if not can_proceed:
            raise RuntimeError(f"Token limit exceeded: {reason}")

        # Truncate messages if they're too large
        ...
        if time_since_last < self.min_request_interval:
            ...
            print(
                f"[openai_client] Throttling: waiting {sleep_time:.1f}s between requests"
            )
            ...
            for attempt in range(1, self.max_retries_per_key + 1):
                try:
                    ...
                    if hasattr(resp, "usage") and resp.usage:
                        ...
                        self.token_usage_data["tokens_used_today"] += tokens_used
                        self.token_usage_data["tokens_used_total"] += tokens_used
                        self.token_usage_data["request_count_today"] += 1
                        self.token_usage_data["request_count_total"] += 1
                        self._save_token_usage_data()

                        print(
                            f"[openai_client] Used {tokens_used} tokens (Total today: {self.token_usage_data['tokens_used_today']})"
                        )

                        # Warn if approaching limit
                        if (
                            self.token_usage_data["tokens_used_today"]
                            > self.daily_token_limit * 0.8
                        ):
                            remaining = (
                            ...
                            print(
                                f"⚠️ [openai_client] Warning: {remaining} tokens remaining today"
                            )
    ...
    def say(self, text: str, **kwargs) -> str:
        """Convenience method for single messages with token tracking."""
...
def get_llm_response(user_input: str, context: list = None) -> str:
    ...
    messages = []
    if context:
        for exchange in context[-3:]:  # Keep only last 3 exchanges
